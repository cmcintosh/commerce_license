...
/**
 * Plugin implementation of the 'commerce_license_attributes_overview' formatter.
 *
 * @FieldFormatter(
 *   id = "commerce_license_attributes_overview",
 *   label = @Translation("License attributes overview"),
 *   field_types = {
 *     "entity_reference",
 *   },
 * )
 */
class LicenseAttributesOverview extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a new LicenseAttributesOverview object.
   *
   ...
   *   The renderer.
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, EntityTypeManagerInterface $entity_type_manager, EntityDisplayRepositoryInterface $entity_display_repository, LicenseAttributeFieldManagerInterface $attribute_field_manager, RendererInterface $renderer) {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $license_type_storage = $this->entityTypeManager->getStorage('commerce_license_type');
    $attribute_storage = $this->entityTypeManager->getStorage('commerce_license_attribute');
    /** @var \Drupal\commerce_license\Entity\LicenseTypeInterface $license_bundle */
    $license_bundle = $license_type_storage->load($this->fieldDefinition->getTargetBundle());
    $attribute_map = $this->attributeFieldManager->getFieldMap($license_bundle->getVariationTypeId());
    $used_attributes = [];
    ...
    $view_modes = $this->entityDisplayRepository->getViewModes('commerce_license_attribute_value');
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    $attribute_storage = $this->entityTypeManager->getStorage('commerce_license_attribute');
    $elements = [];
    /** @var \Drupal\commerce_license\Entity\LicenseAttributeInterface[] $attributes */
    $attributes = $attribute_storage->loadMultiple(array_filter($this->getSetting('attributes')));
   ...
   *   The render array.
   */
  protected function getAttributeItemList(FieldItemListInterface $variation_items, LicenseAttributeInterface $attribute) {
    $build = [
      '#theme' => 'item_list',
    ...
    $view_builder = $this->entityTypeManager->getViewBuilder('commerce_license_attribute_value');
    /** @var \Drupal\Core\Field\Plugin\Field\FieldType\EntityReferenceItem $variation */
    foreach ($variation_items as $variation) {
      /** @var \Drupal\commerce_license\Entity\LicenseAttributeValueInterface $attribute_value */
  ...
  public static function isApplicable(FieldDefinitionInterface $field_definition) {
    ...
    return $entity_type == 'commerce_license' && $field_name == 'variations';
  }
