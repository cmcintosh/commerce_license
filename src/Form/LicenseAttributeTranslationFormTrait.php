...
/**
 * Provides common functionality for the license attribute translation forms.
 */
trait LicenseAttributeTranslationFormTrait {
  ...
  /**
   * Builds the translation form for license attribute values.
   *
   * @param array $form
   ...
   */
  protected function buildValuesForm(array $form, FormStateInterface $form_state, LicenseAttributeInterface $attribute) {
  ...
  /**
   * Renders the given license attribute value in the original language.
   *
   * Skips non-translatable fields. Skips all base fields other than the name.
   *
   * @param \Drupal\commerce_license\Entity\LicenseAttributeValueInterface $value
   *   The license attribute value.
   *
   * @return array
   *   The render array.
   */
  protected function renderOriginalValue(LicenseAttributeValueInterface $value) {
    $value = $value->getUntranslated();
    $view_builder = $this->getEntityTypeManager()->getViewBuilder('commerce_license_variation');
    $build = [];
