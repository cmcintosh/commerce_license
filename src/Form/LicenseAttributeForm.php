...
class LicenseAttributeForm extends BundleEntityFormBase {
  ...
  /**
   * Constructs a new LicenseAttributeForm object.
   *
   * @param \Drupal\commerce_license\LicenseAttributeFieldManagerInterface $attribute_field_manager
   *   The attribute field manager.
   */
  public function __construct(LicenseAttributeFieldManagerInterface $attribute_field_manager) {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\commerce_license\Entity\LicenseAttribute::load',
      ],
    ...
    $form['elementType'] = [
      ...
      '#options' => [
        ...
        'commerce_license_rendered_attribute' => $this->t('Rendered attribute'),
      ],
      '#default_value' => $attribute->getElementType(),
    ];

    // Allow the attribute to be assigned to a license variaton type.
    ...
    $form['variation_types'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('License variation types'),
    ];
    $attribute_field_map = $this->attributeFieldManager->getFieldMap();
    $variation_type_storage = $this->entityTypeManager->getStorage('commerce_license_variation_type');
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if (!$attribute->isNew()) {
        ...
        $enabled = $translation_manager->isEnabled('commerce_license_attribute_value', $attribute->id());
      }
  ...
  public function buildValuesForm(array $form, FormStateInterface $form_state) {
    ...
    $wrapper_id = Html::getUniqueId('license-attribute-values-ajax-wrapper');
    $form['values'] = [
      '#type' => 'table',
      '#header' => [
      ...
      '#tabledrag' => [
        [
          ...
          'group' => 'license-attribute-value-order-weight',
        ],
      ],
      '#weight' => 5,
    ...
    foreach ($value_map as $index => $id) {
      ...
      $value_form['entity'] = [
        ...
        '#entity_type' => 'commerce_license_attribute_value',
        '#bundle' => $attribute->id(),
      ...
      $value_form['weight'] = [
        ...
        '#attributes' => [
          'class' => ['license-attribute-value-order-weight'],
        ],
      ];
    ...
    $access_handler = $this->entityTypeManager->getAccessControlHandler('commerce_license_attribute_value');
  ...
  protected function saveValues(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($delete_queue)) {
      $value_storage = $this->entityTypeManager->getStorage('commerce_license_attribute_value');
    ...
    foreach ($form_state->getValue(['values']) as $index => $value_data) {
      /** @var \Drupal\commerce_license\Entity\LicenseAttributeValueInterface $value */
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if ($translation_manager->isEnabled('commerce_license_attribute_value', $this->entity->id()) != $enabled) {
        $translation_manager->setEnabled('commerce_license_attribute_value', $this->entity->id(), $enabled);
    ...
    if ($status == SAVED_NEW) {
      drupal_set_message($this->t('Created the %label license attribute.', ['%label' => $this->entity->label()]));
      // Send the user to the edit form to create the attribute values.
      $form_state->setRedirectUrl($this->entity->toUrl('edit-form'));
    }
    else {
      $this->saveValues($form, $form_state);
      drupal_set_message($this->t('Updated the %label license attribute.', ['%label' => $this->entity->label()]));
