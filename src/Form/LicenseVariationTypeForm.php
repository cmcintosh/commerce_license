...
class LicenseVariationTypeForm extends BundleEntityFormBase {
  ...
  /**
   * Constructs a new LicenseVariationTypeForm object.
   *
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\commerce_license\Entity\LicenseVariationType::load',
      ],
    ...
    if ($this->moduleHandler->moduleExists('commerce_order')) {
      // Prepare a list of order item types used to purchase license variations.
      ...
      $order_item_types = array_filter($order_item_types, function ($order_item_type) {
        return $order_item_type->getPurchasableEntityTypeId() == 'commerce_license_variation';
      });
    ...
    $attributes = $this->entityTypeManager->getStorage('commerce_license_attribute')->loadMultiple();
    ...
    if ($this->moduleHandler->moduleExists('language')) {
      ...
      $form['language']['language_configuration'] = [
        ...
        '#default_value' => ContentLanguageSettings::loadByEntityTypeBundle('commerce_license_variation', $variation_type->id()),
      ];
  ...
  public function save(array $form, FormStateInterface $form_state) {
    $this->entity->save();
    drupal_set_message($this->t('Saved the %label license variation type.', ['%label' => $this->entity->label()]));
    $form_state->setRedirect('entity.commerce_license_variation_type.collection');
    ...
    if ($selected_attributes) {
      /** @var \Drupal\commerce_license\Entity\LicenseAttributeInterface[] $selected_attributes */
    ...
    if ($unselected_attributes) {
      /** @var \Drupal\commerce_license\Entity\LicenseAttributeInterface[] $unselected_attributes */
